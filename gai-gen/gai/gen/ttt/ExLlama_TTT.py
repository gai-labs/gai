...
class ExLlama_TTT:
    ...
    def classify_text_nature(self, text):
        ...
        # Look for the pattern that matches '{"function":'
        pattern = r'^\s*{\s*(\\n)?\s*(\")?function(\")?\s*:\s*'
        if re.search(pattern, text):
            return "tools"
    ...
    def _generating(self, prompt, **model_params):
        ...
        if chunk_type == 'text':
            ...
            if (not finish_reason):
                raise Exception(f"ExLlama_TTT.generating: 'text' response missing finish_reason.")
        ...
        if chunk_type == 'tool': 
            ...
            if (not function_name):
                raise Exception(f"ExLlama_TTT.generating: 'tool' response missing function_name. output: {text}")

            if (not function_arguments):
                raise Exception(f"ExLlama_TTT.generating: 'tool' response missing function_arguments. output: {text}")
        ...
        raise Exception(f'ExLlama_TTT.generating: Response type cannot be classified. output: {text}')
